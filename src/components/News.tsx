...
export default function News() {
  ...
  // ✅ Fungsi untuk tambah views & tampilkan detail
  const handleReadMore = async (news: NewsItem) => {
    try {
      const res = await axios.put(`http://localhost:5000/api/news/${news._id}/views`)
      setSelectedNews(res.data) // gunakan data baru dengan views yang sudah ditambah
    } catch (err) {
      console.error("Gagal menambahkan views:", err)
      setSelectedNews(news) // fallback jika error
    }
  }
          ...
          {filteredNews.map((news) => (
            <Card key={news._id} className="hover:shadow-lg transition-shadow">
              ...
              <CardContent>
                ...
                <Button onClick={() => handleReadMore(news)} className="w-full bg-[#1e3a8a] hover:bg-blue-600">
